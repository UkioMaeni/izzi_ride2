...
class SearchFromInput extends StatefulWidget {
  const SearchFromInput({super.key});

  @override
  State<SearchFromInput> createState() => _SearchFromInputState();
}

class _SearchFromInputState extends State<SearchFromInput> {
  ...
  Future<void> getMapParams(String placeId,String description)async{
    isLockTaped=true;
    final result = await UserHttp.I.getMapParamsFromGoogle(placeId,description);
  ...
  toMapScene(){
    context.pushNamed(RoutesName.searchFromMap);
    //Navigator.push(context, MaterialPageRoute(builder: (context) => FromMapScene(),));
  }

  setMapParams(MapParams mapParams){
    context.read<SearchRideBloc>().add(SearchRideSetMapParams(mapParams: mapParams,type: SceneType.from));
  }
