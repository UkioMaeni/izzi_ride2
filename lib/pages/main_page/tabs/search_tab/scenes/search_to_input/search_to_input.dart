...
class SearchToInput extends StatefulWidget {
  const SearchToInput({super.key});

  @override
  State<SearchToInput> createState() => _SearchToInputState();
}

class _SearchToInputState extends State<SearchToInput> {
  ...
  toMapScene(){
    context.goNamed(RoutesName.searchToMap);
    //Navigator.push(context, MaterialPageRoute(builder: (context) => FromMapScene(),));
  }

  setMapParams(MapParams mapParams){
    context.read<SearchRideBloc>().add(SearchRideSetMapParams(mapParams: mapParams,type: SceneType.to));
  }
